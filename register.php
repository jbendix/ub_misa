...
<?php
//Gets customer debtorno!!
$grabcustomer = $this->sale_lib_example->get_customer();
...
<?php echo form_input(array('name'=>'item','id'=>'item','size'=>'75'));?>
	...
	<table id="register">
		<thead>
			<tr>
				...
				<th style="width: 30%;"><?php echo "Qty O/H"/*$this->lang->line('sales_item_name')*/; ?></th>
	...
	?>
		...
		<tr>
			...
			<?php
	        	if($item['allow_alt_description']==1)
	        	{
	        		echo form_input(array('name'=>'description','value'=>$item['description'],'size'=>'40'));
	        	}
	        	else
	        	{
					if ($item['description']!='')
...
<div id="overall_sale">
	...
	if(isset($customer))
	{
		...
		//pull customer info from people view 
		$sql = "SELECT last_name, address_1,city,state,zip,phone_number,lastpaid,lastpaiddate
				FROM `ospos_people_view`
				WHERE person_id = '". $grabcustomer ."'";
		...
		echo "<br />Phone: " . $phone_number;
		echo "<br /><br />";
		echo "Last Customer Transaction:";
		if (isset($lastpaiddate)) {
		echo "<br />" . $lastpaiddate;
		echo "<br />$" . $lastpaid;} else {
		echo "<br />No Prior Transactions";
		}
		echo "</strong>";
		echo "<br />";
	...
	else
	{
		...
		<label id="customer_label" for="customer"><?php echo /*$this->lang->line('sales_select_customer')*/"Select Customer"; ?></label>
	...
	<div id='sale_details'>
		...
		<br />
		<?php
		//CODE HERE if customer display is wanted under the already displayed customer
...
var x; // global declaration for use in customer lookup
...
$(document).ready(function()
{
    ...
    $('#select_customer_form').submit(function()
    {
    	var input = $('#select_customer_form').find('input[id="customer"]').val();
    	//alert(input);

    	//$.post('/application/views/sales/getcustomerinfo.php', {customer : input}, function(data){
	 //      alert(data);
...
function check_payment_type_gifcard()
{
	...
	//$("#select_customer_form").submit(function ()) {
			//var x = $(this).val(); // Holds debtorno!
		//	alert('Form Submitted');

			//$.post('gettaxid.php', {taxinput : x}, function(data) {
					//$("#taxoutput").text(((data*100).toFixed(3)) + '%');
				//});
...
<?php $this->load->view("partial/footer"); ?><?php //echo $this->lang->line('sales_giftcard'); ?>
